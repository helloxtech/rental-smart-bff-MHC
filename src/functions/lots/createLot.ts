 ...
 */
export async function createLot(request: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit>
{
    ...
    try
    {
        ...
        {
            hx_lotnumber: String(payload.lotNumber ?? ""),
            hx_status: Number(payload.status ?? 0),
            hx_monthlyrent: Number(payload.monthlyRent ?? 0),
            "hx_CommunityId@odata.bind": `/hx_communities(${communityId})`
        };

        const client: DataverseClient = createClient();
        const created: any = await client.create("hx_lots", body);
        return { status: 201, jsonBody: created ?? { success: true } };
    }
    catch (error)
    {
        return errorResponse(500, "LOT_CREATE_FAILED", (error as Error).message, communityId);
    }
}
